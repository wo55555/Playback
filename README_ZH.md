...
`v0.3.1` 是同时面向 MC 26.10 和 MC 26.20 的编辑器版本。相机轨道以路径叠层的形式绘制在视口中；编辑器布局围绕居中的播放控制条重建，时间轴上标出导出区间；播放速度与界面缩放可直接选择。本版还扩充了键盘编辑与相机切换，改善预览时的实体姿态插值和乘客跟随，并修复维度边界、区块列不完整、点击关键帧误移动及玩家列表缺失导致的回放问题。MC 26.20 版本还修复了跨维度时录制被取消的问题，并保留可移植方块更新和实体名称标签元数据。
