...
`v0.3.1-mc26.40` 的 2026 年 9 月 20 日热修复解决了录制玩家在回放中站立不动、传送数据包导致回放断开连接，以及向后跳转时在已加载过的地形上卡住的问题；同时保留玩家原名，并在录制的玩家列表可用时恢复现有录制中受影响的名称。重新安装同一发行版本即可获取这些修复，无需为此重新录制或转换格式。本次热修复仅适用于 MC 26.40，现有 MC 26.10 和 MC 26.20 发行附件保持不变。
...
Playback 针对不同 Minecraft 与 LeviLamina 版本维护独立发行线。产品版本 `0.3.1` 已面向三个运行时发布，三者均为预发布版本。
