...
`v0.3.1` 是面向 MC 26.10、MC 26.20 和 MC 26.40 的编辑器预发布版本。相机轨道以路径叠层的形式绘制在视口中；编辑器布局围绕居中的播放控制条重建，时间轴上标出导出区间；播放速度与界面缩放可直接选择。本版还扩充了键盘编辑与相机切换，改善预览时的实体姿态插值和乘客跟随，并修复维度边界、区块列不完整、点击关键帧误移动及玩家列表缺失导致的回放问题。MC 26.20 版本还修复了跨维度时录制被取消的问题，并保留可移植方块更新和实体名称标签元数据。

**2026 年 9 月 20 日的 `v0.3.1-mc26.40` 热更新**修复了录制玩家不动、传送数据包导致回放退出，以及回退时已加载地形未被替换而卡住的问题。同时保留玩家原名，并在已保存的玩家列表中存在对应名字时恢复受影响的旧录制名字。重新安装同一发行版本即可更新；本次修复无需重新录制或转换格式。此次热更新仅适用于 MC 26.40，已发布的 MC 26.10 和 MC 26.20 发行资产不变。
...
Playback 针对不同 Minecraft 与 LeviLamina 版本维护独立发行线。产品版本 `0.3.1` 在当前分支发布为 `v0.3.1-mc26.20`；MC 26.10 和 MC 26.40 请使用下表对应的发行版本。三个版本均为预发布版本。
...
Minecraft/LeviLamina `26.10.*` 使用 `v0.3.1-mc26.10`，`26.20.*` 使用 `v0.3.1-mc26.20`，`26.40.*` 使用 `v0.3.1-mc26.40`。上述三个版本均为预发布版本。不要混装不同发行线，参见[兼容性](#兼容性)。
