<div align="center">
  ...
  <p>
    <a href="https://discord.gg/qKrT38mFh7"><img src="https://img.shields.io/discord/1531092797367652403?style=for-the-badge&amp;logo=discord" alt="Playback Discord 在线人数"></a>
...
> [!WARNING]
> Playback 目前仍处于早期开发阶段，现有公开版本均为预发布版本。请备份重要世界和录制文件；在 Minecraft、LeviLamina 或 Playback 版本发生变化后，不保证旧回放仍然兼容。
...
- **时间线编辑器** — 提供可缩放轨道、可调整面板、相机/序列/实体片段编辑，以及撤销与重做；工程随回放数据保存，重新打开时自动恢复。
...
`v0.3.1` 是面向 MC 26.10、MC 26.20 和 MC 26.40 的编辑器版本，三者均为预发布版本。相机轨道以路径叠层的形式绘制在视口中；编辑器布局围绕居中的播放控制条重建，时间轴上标出导出区间；播放速度与界面缩放可直接选择。本版还扩充了键盘编辑与相机切换，改善预览时的实体姿态插值和乘客跟随，并修复维度边界、区块列不完整、点击关键帧误移动及玩家列表缺失导致的回放问题。

`v0.3.1-mc26.40` 的 2026 年 9 月 20 日热修复解决了录制玩家站立不动、回放传送包时断开连接，以及向后跳转时卡在此前已加载地形的问题；同时保留玩家原名，并从已录制的玩家列表中恢复现有回放内受影响的名字。重新安装同一发行版本即可获取这些修复；针对这些问题，无需重新录制或转换格式。本次热修复仅适用于 MC 26.40，现有 MC 26.10 和 MC 26.20 发行资产保持不变。

> [!CAUTION]
> Playback 当前发布的仍是预发布版本，可能直接进行破坏性的格式或配置更新。早于 `v0.2.0-mc26.10` 的版本创建的回放不兼容，必须重新录制。受影响服务器在 2026 年 8 月 20 日热更新前录制的回放可能已经缺少可移植区块或自定义实体注册数据，这类归档无法修复，也必须重新录制；数据完整的 `v0.2.x-mc26.10` 和 `v0.3.0-mc26.10` 回放与 `v0.3.1-mc26.10` 兼容，无需转换。配置版本、录制文件的快照上下文版本和编辑器工程格式版本均为 `1`，不提供迁移层。
...
Playback 针对不同 Minecraft 与 LeviLamina 版本维护三条独立发行线。产品版本 `0.3.1` 已面向 MC 26.10、MC 26.20 和 MC 26.40 发布，三者均为预发布版本。`main` 源码分支仍面向 MC 26.10 构建；构建其他运行时版本需使用对应分支。
...
`v0.3.1-mc26.10` 提供实验性的 H.264 MP4 和 PNG 序列导出。目前仍存在已知限制，并且不包含音频。
...
Minecraft / LeviLamina `26.10.*` 使用 `v0.3.1-mc26.10`，`26.20.*` 使用 `v0.3.1-mc26.20`，`26.40.*` 使用 `v0.3.1-mc26.40`。三者均为预发布版本。三条发行线独立维护，请勿混装。

## 开发状态与计划

- 录制、回放、相机和导出工作流仍在持续构建与优化中。
- 后续将重点调试多人服务器会话的录制与回放，欢迎测试并反馈问题。
- 后续计划包括音频导出、更广泛的渲染器兼容和更多相机工具。
