...
请选择与目标 Minecraft / LeviLamina 运行时对应的分支：`main` 和 `develop` 对应 `26.10`，`feat/26.20` 对应 `26.20`，`feat/26.40` 对应 `26.40`（预发布）。`PLAYBACK_VERSION` 仅修改打包版本标识，不会切换 SDK 目标。以下命令和清单核验仍针对本分支的 `26.10` 目标。
