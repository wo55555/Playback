...
请选择与目标 Minecraft / LeviLamina 运行时对应的分支：`main` 和 `develop` 对应 `26.10`，`feat/26.20` 对应 `26.20`，`feat/26.40` 对应 `26.40`。当前面向这三个运行时的 Playback 版本均为预发布版本。`PLAYBACK_VERSION` 仅修改打包版本标识，不会切换 SDK 目标。以下命令和清单核验仍针对本分支的 `26.10` 目标。
...
构建完成后，请确认 `bin/playback/manifest.json` 显示 `0.3.1-mc26.10`、`bin/playback/tools/ffmpeg.exe` 存在，并执行 `git diff --check`。涉及运行时行为的发行版本还应在支持的渲染路径上分别导出短 PNG 序列和 MP4。
