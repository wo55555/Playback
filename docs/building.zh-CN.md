...
MC 26.10、MC 26.20 和 MC 26.40 当前对应的 Playback 发行版本均为预发布版本。
...
构建完成后，请确认 `bin/playback/manifest.json` 显示 `0.3.1-mc26.40`、`bin/playback/tools/ffmpeg.exe` 存在，并执行 `git diff --check`。涉及运行时行为的发行版本还应在支持的渲染路径上分别导出短 PNG 序列和 MP4。
