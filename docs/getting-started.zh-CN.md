...
上表中的三个 Playback 版本均为预发布版本。三条发行线独立维护，请勿跨运行时混装。
...
> [!CAUTION]
> `v0.3.1-mc26.10` 仍是预发布版本。早于 `v0.2.0-mc26.10` 的版本创建的回放不兼容，必须重新录制。受影响服务器在 2026 年 8 月 20 日热更新前录制的回放可能已经缺少可移植区块或自定义实体注册数据，这类归档无法修复，也必须重新录制；数据完整的 `v0.2.x-mc26.10` 和 `v0.3.0-mc26.10` 回放与 `v0.3.1-mc26.10` 兼容，无需转换。配置版本、录制文件的快照上下文版本和编辑器工程格式版本均为 `1`，不提供迁移。
...
| `Ctrl+S`                        | 保存工程              |
| `Ctrl+E`                        | 打开导出设置          |
| `Ctrl+Shift+F`                  | 最大化 / 还原视口     |
| `Ctrl+Shift+-` / `Ctrl+Shift+=` | 缩小 / 放大界面       |
| `Ctrl+Shift+0`                  | 重置界面缩放          |
| `Esc`（长按 0.8 秒）            | 退出回放并保存        |

编辑器打开时键盘归编辑器所有。在游戏视口上按住鼠标左键可操控观察者相机，按住期间快捷键暂停响应。
