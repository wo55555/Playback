...
```powershell
$env:PLAYBACK_VERSION = 'v0.3.1-mc26.40'
...
After building, verify `bin/playback/manifest.json` reports `0.3.1-mc26.40`, confirm `bin/playback/tools/ffmpeg.exe` exists, and run `git diff --check`. Runtime-sensitive releases should also export a short PNG sequence and MP4 on the supported renderer paths.
